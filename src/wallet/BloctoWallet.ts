...
export class BloctoWalletAdapter extends BaseWalletAdapter {
  ...
  async signAndSubmitTransaction(transaction: Types.TransactionPayload): Promise<{ hash: Types.HexEncodedBytes }> {
    try {
      try {
        const provider = this._provider
        const response = await provider?.signAndSubmitTransaction(transaction as Types.EntryFunctionPayload)
        if (response) {
