...
interface IOkxWallet {
  connect: (options?: IConnectOptions) => Promise<AddressInfo>
  account: () => Promise<AddressInfo>
  isConnected: () => Promise<boolean>
  signAndSubmitTransaction(transaction: any, options?: any): Promise<{ hash: Types.HexEncodedBytes } | IApotsErrorResult>
...
export class OkxWalletAdapter extends BaseWalletAdapter {
  ...
  icon =
    ...
    'https://lh3.googleusercontent.com/sO4GPPbgvTCXEFWp-uALYYju9vxVOr5YSr2jqRAclNFSq8FrAIVTQOYmpJV4kMuDM1ZazcgUdjNGLIGKpmMRvR3NzQ=w128-h128-e365-rj-sc0x00ffffff'
  ...
  async signAndSubmitTransaction(transaction: Types.TransactionPayload, options?: any): Promise<{ hash: Types.HexEncodedBytes }> {
    try {
      const wallet = this._wallet
      const provider = this._provider || window.okxwallet?.aptos
      if (!wallet || !provider) throw new WalletNotConnectedError()
