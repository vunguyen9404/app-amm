...
import { AccountKeys, BaseWalletAdapter, scopePollingDetectionStrategy, WalletName, WalletReadyState } from './BaseAdapter'
...
export class HippoExtensionWalletAdapter extends BaseWalletAdapter {
  ...
  async signAndSubmitTransaction(transaction: TransactionPayload): Promise<{ hash: HexEncodedBytes }> {
    try {
      const wallet = this._wallet
      if (!wallet) throw new WalletNotConnectedError()
