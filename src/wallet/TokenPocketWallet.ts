...
interface ITokenPocketWallet {
  ...
  signAndSubmitTransaction(transaction: TransactionPayload, options?: any): Promise<{ hash: HexEncodedBytes }>
...
export class TokenPocketWalletAdapter extends BaseWalletAdapter {
  ...
  async signAndSubmitTransaction(transaction: TransactionPayload, options?: any): Promise<{ hash: HexEncodedBytes }> {
    try {
      const wallet = this._wallet
      const provider = this._provider || window.aptos
      if (!wallet || !provider) throw new WalletNotConnectedError()
