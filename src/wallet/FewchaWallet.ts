...
export class FewchaWalletAdapter extends BaseWalletAdapter {
  ...
  protected _wallet: any | null | any
  ...
  async connect(): Promise<void> {
    try {
      ...
      const provider = this._provider || window?.fewcha?.aptos
  ...
  async signTransaction(transaction: Types.TransactionPayload, options?: any): Promise<Uint8Array> {
    try {
      ...
      const provider = this._provider || window?.fewcha?.aptos
      const tx = await provider.generateTransaction(transaction as Types.EntryFunctionPayload, options)
  ...
  async signMessage(msgPayload: SignMessagePayload | any): Promise<SignMessageResponse | any> {
    try {
      const wallet = this._wallet
      const provider = this._provider || window?.fewcha?.aptos
